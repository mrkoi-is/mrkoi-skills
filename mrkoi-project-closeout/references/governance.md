...
| 授权和发布范围 | “仅清理已授权对象”“本次属于发布收尾” | 当前用户请求、具体候选、适用的项目规则；仅在该阶段核对发布凭证 |
...
- **跨项目变更**：先确认任务已包含的实际 upstream/consumer；范围外只报告依赖证据，不因为引用就读取其他私人项目全文。
...
必须同时满足：在请求范围内、安全、可逆、没有外部副作用，并且用户当前请求已授权此类修复。例如：
...
- 缺少具体授权的目录或项目重命名、文件/目录删除；已有明确授权则完成核验后执行，无须固定的二次确认；
...
- **无门禁**：同一违规第三次出现，说明散文约束不足。仅在本次改进范围内建议或实现 check/hook/test，不因次数自动增加流程。
...
本地收尾不主动检索法律、隐私政策或平台发布规范；工程凭证和秘密保护按当前任务实际需要检查。平台记忆只有用户直接要求更新后才按宿主入口维护，项目规则不能代替这项授权。
