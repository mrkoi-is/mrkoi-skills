#!/usr/bin/env bash
# Read-only inventory for mrkoi-project-closeout. Prints metadata and paths, not file contents.

set -euo pipefail

usage() {
  echo "usage: $0 [--local-only] <project-root>" >&2
  exit 64
}
...
printf '# mrkoi-project-closeout inventory v3\n'
...
if git -C "$PROJECT_ROOT" rev-parse --show-toplevel >/dev/null 2>&1; then
  ...
  printf 'head=%s\n' "$(git -C "$PROJECT_ROOT" rev-parse --verify HEAD 2>/dev/null || printf unborn)"
  printf 'status_entries=%s\n' "$(git --no-optional-locks -C "$PROJECT_ROOT" status --porcelain=v1 | wc -l | tr -d ' ')"
